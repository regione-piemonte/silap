 ...
 * =========================LICENSE_END==================================
 */
package it.csi.silap.colmirbff.integration.dao;
...
import it.csi.silap.colmirbff.util.reflection.ReflectionUtils;
...
public class DAO extends PanacheEntityBase implements Serializable {
	...
	public <E> SqlComplexResult<E> findNativeQuerySql(String sqlSelect, Map<String, Object> params, String sqlOrder, int page, Class<E> resultCustomEntityClass) {
		return findNativeQuerySql(sqlSelect,params, sqlOrder, page, SilapConstants.NUMERO_RECORD_PER_PAGINA, resultCustomEntityClass);
	}

	
	public <E> SqlComplexResult<E> findNativeQuerySql(String sqlSelect, Map<String, Object> params, String sqlOrder, int page, int recordOfPage, Class<E> resultCustomEntityClass) {
		
		SqlComplexResult<E> ris = new SqlComplexResult<E>();
		
		Long count = getCountNative("select count(*) from (" + sqlSelect +  ") as numRecord ", params);
		int numTotalPage = 0;
		List<E> list = new ArrayList<E>();
		
		
		if (page<0) {
			if (sqlOrder == null) sqlOrder = "";
			list = findNativeQuerySql(sqlSelect + sqlOrder, params, resultCustomEntityClass);
		}
		else {
			if (count <= recordOfPage) {
				list = findNativeQuerySql(sqlSelect, params, resultCustomEntityClass);
				numTotalPage = 1;
			} else {
				String limitSql = " LIMIT " + recordOfPage + " OFFSET "
						+ (page * recordOfPage);
				
				if (sqlOrder == null) sqlOrder = "";
				
				list = findNativeQuerySql(sqlSelect + sqlOrder + limitSql, params, resultCustomEntityClass);
				
				numTotalPage = (int)(count/recordOfPage);
				
				if ((count % recordOfPage)>0)
					numTotalPage++;
			}
		}
		...
		ris.setList(list);
		ris.setRecordOfPage(list.size());
		ris.setTotalPage(numTotalPage);
		ris.setTotalResult(count.intValue());
